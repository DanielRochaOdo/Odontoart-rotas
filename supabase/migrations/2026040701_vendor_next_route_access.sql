alter table public.profiles
  add column if not exists can_access_next_route_dashboard boolean;

update public.profiles
set can_access_next_route_dashboard = false
where can_access_next_route_dashboard is null;

alter table public.profiles
  alter column can_access_next_route_dashboard set default false,
  alter column can_access_next_route_dashboard set not null;
...
declare
  ...
  resolved_can_access_next_route_dashboard boolean;
begin
  ...
  resolved_can_access_next_route_dashboard := case
    when lower(coalesce(new.raw_user_meta_data->>'can_access_next_route_dashboard', '')) in ('1', 't', 'true', 'y', 'yes', 'on')
      then true
    else false
  end;
  ...
  if desired_role <> 'VENDEDOR' then
    ...
    resolved_can_access_next_route_dashboard := false;
  end if;
  ...
  insert into public.profiles (
    ...
    can_access_next_route_dashboard,
    supervisor_id,
    vendedor_id
  )
  values (
    ...
    resolved_can_access_next_route_dashboard,
    supervisor_uuid,
    vendedor_uuid
  )
  on conflict (user_id) do update
    set role = excluded.role,
        ...
        can_access_next_route_dashboard = excluded.can_access_next_route_dashboard,
