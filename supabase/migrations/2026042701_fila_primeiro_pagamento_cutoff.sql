...
-- somente empresas com AnoMesPrimeiroPagamento >= 2026-01.
...
alter table public.queue_release_controls
  ...
  check (data_contrato >= date '2026-01-01');
...
begin
  ...
  if p_data_contrato is null then
    raise exception 'AnoMesPrimeiroPagamento obrigatorio.';
  end if;

  if p_data_contrato < date '2026-01-01' then
    raise exception 'Empresa fora do escopo do modulo fila (AnoMesPrimeiroPagamento anterior a 2026-01).';
  end if;
